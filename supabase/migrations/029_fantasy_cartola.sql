...
BEGIN
  ...
  SELECT fantasy_season_id, player.id, (SELECT initial_player_price FROM public.fantasy_seasons WHERE id = fantasy_season_id)
  ...
  VALUES (fantasy_season_id, current_round.id, to_jsonb(current_settings) - 'league_id' - 'updated_at')
  ON CONFLICT (round_id) DO UPDATE SET round_id = EXCLUDED.round_id
  RETURNING id INTO fantasy_round_id;
