...
import { repairLegacySavedFantasyLineups, reprocessFantasyRound, reprocessFantasyRoundWithCurrentRules, updateFantasySettings } from "@/lib/actions/fantasy";
...
const fields = [
  ...
  ["competitive_price_floor", "Piso da curva justa"], ["competitive_price_ceiling", "Teto da curva justa"],
  ["competitive_price_curve", "Curva de concentração"], ["market_round_weight", "Peso da rodada atual"],
  ["market_reprice_strength", "Velocidade até o preço justo"], ["market_initial_up_cap", "Limite de alta na R1"],
  ["market_initial_down_cap", "Limite de queda na R1"], ["market_cap_step", "Passo por rodada"],
  ["market_difficulty_min", "V11: dificuldade mínima"], ["market_difficulty_max", "V11: dificuldade máxima"],
  ["market_difficulty_step", "V11: passo da dificuldade"], ["market_target_elite_affordability", "V11: acesso alvo à elite"],
  ["market_target_median_elite_ratio", "V11: patrimônio mediano ÷ elite"], ["market_recovery_bonus_strength", "V11: força de recuperação"],
  ["market_expensive_risk_strength", "V11: risco do jogador caro"], ["market_breakout_reprice_strength", "V11: reação no breakout"],
] as const;
...
export function FantasyAdminSettings({ settings, rounds }: { settings: any; rounds: any[] }) {
  ...
  function repairLegacy() {
    ...
    startTransition(async () => {
      ...
      if (!result.success) {
        setMessage(result.error || "Erro ao reparar escalações antigas.");
        return;
      }
      const names = (result.lineups || []).map((item: any) => `${item.playerName} (R${item.roundNumber})`).join(", ");
      setMessage(result.repaired
        ? `${result.repaired} escalação(ões) reparada(s): ${names}.`
        : "Auditoria concluída: nenhuma escalação completa ficou de fora.");
    });
  }
  return <div className="space-y-6"><section className="glass-card p-4"><div className="mb-4 rounded-xl border border-accent/20 bg-accent/5 p-3"><p className="text-xs font-black text-accent">Mercado V11 · dificuldade adaptativa</p><p className="mt-1 text-[10px] leading-relaxed text-muted">O preço segue 70% a rodada e 30% a temporada. A elite encarece conforme a economia global, enquanto apostas baratas que entregam acima da expectativa recuperam mais patrimônio.</p><Link href="/admin/pontuacao" className="mt-2 inline-block text-[10px] font-black text-accent underline">Editar os 8 scouts básicos na Pontuação BQ</Link></div><div className="grid gap-3 sm:grid-cols-2">{fields.map(([key, label]) => <label key={key}><span className="mb-1 block text-[10px] font-bold text-muted">{label}</span><input type="number" step="0.01" value={values[key]} onChange={e => setValues(v => ({ ...v, [key]: e.target.value }))} className="h-11 w-full rounded-xl border border-border bg-background px-3 text-sm font-bold text-foreground"/></label>)}</div><button onClick={save} disabled={pending} className="mt-4 w-full rounded-xl bg-accent py-3 text-sm font-black text-background disabled:opacity-50">Salvar regras futuras</button><p className="mt-2 text-[10px] text-muted">As alterações valem somente para rodadas abertas depois de salvar. Rodadas antigas usam o snapshot original.</p></section><section><h2 className="mb-2 text-xs font-black uppercase text-muted">Reprocessamento</h2><div className="mb-3 rounded-xl border border-warning/25 bg-warning/10 p-3"><p className="text-[10px] font-black uppercase text-warning">Correção histórica</p><p className="mt-1 text-[10px] leading-relaxed text-muted">“Reprocessar” mantém as regras antigas. “Aplicar regras atuais” troca o snapshot da rodada e refaz pontos, preços e patrimônios a partir dela.</p></div><button type="button" disabled={pending} onClick={repairLegacy} className="mb-3 w-full rounded-xl border border-accent/35 bg-accent/10 px-3 py-3 text-xs font-black text-accent disabled:opacity-40">Auditar e reparar escalações antigas</button><div className="space-y-2">{rounds.map((item: any) => <div key={item.id} className="glass-card space-y-3 p-4"><div className="flex items-center justify-between gap-3"><div><p className="text-sm font-black text-foreground">Rodada {item.round?.number}</p><p className="text-[10px] text-muted">{item.round?.date} · {item.market_status} · regras V{item.rules_version || 0} · pontos V{item.scoring_version || 1}</p></div></div><div className="grid grid-cols-2 gap-2"><button disabled={pending || item.round?.status !== "finished"} onClick={() => reprocess(item.round?.id)} className="min-h-11 rounded-lg border border-warning/30 px-2 py-2 text-[10px] font-black text-warning disabled:opacity-30">Reprocessar igual</button><button disabled={pending || item.round?.status !== "finished"} onClick={() => upgradeRules(item.round?.id)} className="min-h-11 rounded-lg bg-accent px-2 py-2 text-[10px] font-black text-background disabled:opacity-30">Aplicar regras atuais</button></div></div>)}</div></section>{message && <p role="status" className="rounded-xl border border-border bg-surface p-3 text-xs font-bold text-foreground">{message}</p>}</div>;
}
