...
const fields = [
  ...
  ["goalkeeper_appearance_points", "Goleiro: bônus por partida"], ["goal_conceded_points", "Goleiro: pontos por gol sofrido"],
...
export function FantasyAdminSettings({ settings, rounds }: { settings: any; rounds: any[] }) {
  ...
  function save() { startTransition(async () => { const raw = Object.fromEntries(Object.entries(values).map(([key, value]) => [key, Number(value)])); const payload = { ...raw, lossPoints: raw.loss_points, goalkeeperAppearancePoints: raw.goalkeeper_appearance_points, goalConcededPoints: raw.goal_conceded_points }; delete (payload as any).loss_points; delete (payload as any).goalkeeper_appearance_points; delete (payload as any).goal_conceded_points; const result = await updateFantasySettings(payload as any); setMessage(result.success ? "Configurações salvas para as próximas rodadas." : result.error || "Erro ao salvar."); }); }
