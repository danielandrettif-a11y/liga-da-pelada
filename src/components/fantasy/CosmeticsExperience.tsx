...
export function CosmeticsCollection({ cosmetics, playerId, playerName = "Jogador", avatarUrl }: { cosmetics: CosmeticsDashboard; playerId: string; playerName?: string; avatarUrl?: string | null }) {
  ...
  // O provador administrativo mostra apenas o catalogo vigente do Passe.
  // Itens legados continuam no inventario normal para nao invalidar premios
  // ja conquistados por usuarios.
  const sourceItems = (previewMode ? cosmetics.previewCatalog : cosmetics.cosmetics)
    .filter((item) => !isLegacyFrameAsset(item.slot === "frame" ? item.assetKey : null));
  const activeLoadout = previewMode ? previewLoadout : cosmetics.equipped;
  // Mesmo fora do catalogo, uma moldura legada equipada deve continuar
  // aparecendo no avatar do provador até o usuario escolher outra.
  const activeItems = [...sourceItems, ...(previewMode ? cosmetics.cosmetics : [])]
    .filter((item, index, items) => activeLoadout[item.slot] === item.id && items.findIndex((candidate) => candidate.id === item.id) === index);
  ...
  function togglePreview() {
    ...
    setCartolaPreview(false);
    setPreviewMode((current) => !current);
    setMessage("");
  }

  function openOfficialPreview(slot?: CosmeticSlot, cosmeticId?: string) {
    const nextLoadout = slot && cosmeticId
      ? { ...previewLoadout, [slot]: cosmeticId }
      : previewLoadout;
    if (slot && cosmeticId) setPreviewLoadout(nextLoadout);

    const params = new URLSearchParams({ preview: "admin" });
    for (const [loadoutSlot, selectedId] of Object.entries(nextLoadout)) {
      if (selectedId) params.set(loadoutSlot, selectedId);
    }
    router.push(`/jogadores/${playerId}?${params.toString()}`);
  }
    ...
    <section className="space-y-3">
      <div className="overflow-hidden rounded-3xl border border-accent/25 bg-gradient-to-br from-[#123421] via-[#06150d] to-[#0d1811]">
        <div className="relative p-5">
          ...
          <div className={`relative mt-4 grid gap-2 ${previewMode ? "sm:grid-cols-3" : "sm:grid-cols-2"}`}>
            ...
            {previewMode ? <button type="button" onClick={() => openOfficialPreview()} className="w-full rounded-xl border border-amber-300/55 bg-amber-300/15 px-3 py-2.5 text-[10px] font-black uppercase text-amber-100 transition-colors hover:bg-amber-300 hover:text-background">Ver na página oficial do Elenco</button> : <button type="button" onClick={() => setProfilePreview((current) => !current)} className={`w-full rounded-xl border px-3 py-2.5 text-[10px] font-black uppercase transition-colors ${profilePreview ? "border-accent/60 bg-accent text-background" : "border-white/20 bg-black/20 text-white"}`}>{profilePreview ? "Voltar à prévia rápida" : "Ver perfil completo"}</button>}
            {previewMode && <button type="button" onClick={() => setCartolaPreview((current) => !current)} className={`w-full rounded-xl border px-3 py-2.5 text-[10px] font-black uppercase transition-colors ${cartolaPreview ? "border-accent/60 bg-accent text-background" : "border-white/20 bg-black/20 text-white"}`}>{cartolaPreview ? "Fechar prévia do Cartola" : "Ver ficha no Cartola"}</button>}
          </div>
        </div>
      </div>
      ...
      <div className={`relative overflow-hidden rounded-3xl border ${previewMode ? "border-amber-300/55" : "border-white/10"} bg-gradient-to-br ${cosmeticVisual(heroAsset)} shadow-2xl`} style={heroImage ? { backgroundImage: `${banner ? "linear-gradient(115deg,rgba(2,14,8,.5),rgba(2,14,8,.08))" : "linear-gradient(115deg,rgba(2,14,8,.22),rgba(2,14,8,.02))"}, url(${heroImage})`, backgroundSize: "cover", backgroundPosition: cosmeticBackgroundPosition(banner ? "banner" : "background", heroAsset) } : undefined}>
        ...
        <div className="relative flex min-h-48 flex-col items-center justify-center p-6 text-center">
          ...
          <CosmeticNameplate assetKey={nameplate?.assetKey} playerName={playerName} titleName={title?.name} className="mt-4 w-full max-w-[18rem] backdrop-blur-sm" />
        </div>
        {profilePreview && <div className="relative border-t border-white/10 bg-[#031109]/75 p-4 backdrop-blur-sm"><p className="text-center text-[9px] font-black uppercase tracking-[.18em] text-accent">Prévia do perfil completo</p><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">{[["PONTOS", "29"], ["JOGOS", "9"], ["GOLS", "3"], ["ASSISTS", "4"]].map(([label, value]) => <div key={label} className="rounded-xl border border-accent/15 bg-black/20 p-2 text-center"><p className="text-[8px] font-black text-muted">{label}</p><p className="mt-1 text-sm font-black text-white">{value}</p></div>)}</div><div className="mt-3 rounded-xl border border-white/10 bg-black/20 p-3"><p className="text-[9px] font-black uppercase tracking-wider text-muted">Identidade do jogador</p><CosmeticNameplate assetKey={nameplate?.assetKey} playerName={playerName} titleName={title?.name} className="mx-auto mt-2 w-full" /><p className="mt-2 text-[10px] leading-4 text-muted">Esta é a mesma assinatura visual usada no perfil, no provador e nas fichas do ranking.</p></div></div>}
      </div>

      {previewMode && cartolaPreview && (
        <section className="rounded-3xl border border-amber-300/30 bg-[#06160d] p-3 shadow-2xl">
          <div className="mb-2 flex items-center justify-between gap-3 px-1">
            <div><p className="text-[9px] font-black uppercase tracking-[.16em] text-amber-200">Prévia da ficha no Cartola</p><p className="mt-0.5 text-[10px] text-muted">A capa e a moldura acompanham a combinação temporária acima.</p></div>
          ...
          <div className="relative min-h-[170px] overflow-hidden rounded-2xl border border-accent/40 bg-[#06160d] p-5">
            ...
            <div className="relative flex items-center gap-3.5">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2"><h3 className="truncate text-lg font-black text-white">{playerName}</h3><span className="shrink-0 rounded-full bg-accent/15 px-2 py-0.5 text-[9px] font-black uppercase text-accent">Jogador</span></div>
                <p className="mt-0.5 text-xs text-white/65">8 gols · 6 assistências · 9 vitórias</p>
                <CosmeticNameplate assetKey={nameplate?.assetKey} playerName={playerName} titleName={title?.name} compact className="mt-2 max-w-[15rem]" />
                <div className="mt-2 flex flex-wrap gap-1"><span className="rounded-md border border-accent/40 bg-accent/20 px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wider text-accent">👑 Mais escalado</span><span className="rounded-md border border-warning/40 bg-warning/20 px-1.5 py-0.5 text-[8px] font-black uppercase tracking-wider text-warning">🎯 Artilheiro em forma</span></div>
              </div>
            </div>
          </div>
        </section>
      ...
      {visible.length ? <div className="grid gap-3 sm:grid-cols-2">{visible.map((item) => {
        ...
        const frameImage = item.slot === "frame" ? cosmeticFrameImage(item.assetKey) : null;
        const isAura = item.slot === "aura";
        const isNameplate = item.slot === "nameplate";
        const isTitle = item.slot === "title";
        return <article key={item.id} className={`relative overflow-hidden rounded-2xl border bg-surface p-4 ${rarityClass(item.rarity)} ${selected ? "ring-1 ring-accent/70" : ""}`}><div className={`absolute inset-x-0 top-0 h-36 overflow-hidden bg-gradient-to-r opacity-95 ${cosmeticVisual(item.assetKey)}`} style={image ? { backgroundImage: `${item.slot === "banner" ? "linear-gradient(rgba(3,14,8,.01), rgba(3,14,8,.16))" : item.slot === "background" ? "linear-gradient(rgba(3,14,8,.01), rgba(3,14,8,.18))" : "linear-gradient(rgba(3,14,8,.05), rgba(3,14,8,.38))"}, url(${image})`, backgroundSize: "cover", backgroundPosition: cosmeticBackgroundPosition(item.slot, item.assetKey) } : undefined}>{isTitle && <CosmeticTitlePreview item={item} className="absolute inset-0" />}{isNameplate && <div className="absolute inset-0 flex items-center justify-center bg-black/25 px-5"><CosmeticNameplate assetKey={item.assetKey} playerName={playerName} titleName={title?.name} className="w-full" /></div>}{(frameImage || isAura) && <div className="absolute inset-0 flex items-center justify-center bg-[radial-gradient(circle_at_center,rgba(77,132,82,.38),rgba(3,15,8,.9)_68%)]"><PlayerAvatar name={playerName} avatarUrl={avatarUrl} clickable={false} frameKey={frameImage ? item.assetKey : null} auraKey={isAura ? item.assetKey : null} className="h-20 w-20 rounded-full bg-[#102819] text-sm font-black text-white" /></div>}<div className="cosmetic-card-shimmer absolute inset-0" /></div><div className="relative"><div className="flex items-start justify-between gap-2"><span className="rounded-md bg-black/65 px-2 py-1 text-[8px] font-black uppercase backdrop-blur-sm">{COSMETIC_SLOT_LABELS[item.slot]} · {item.rarity}</span>{selected && <span className="rounded-md bg-accent px-2 py-1 text-[8px] font-black text-background">{previewMode ? "Na prévia" : "Equipado"}</span>}</div><h3 className="mt-20 text-sm font-black text-foreground">{item.name}</h3><p className="mt-1 min-h-8 text-[11px] leading-4 text-muted">{item.description}</p>{previewMode ? <button disabled={selected} onClick={() => setPreviewLoadout((current) => ({ ...current, [item.slot]: item.id }))} className="mt-3 w-full rounded-xl border border-amber-300/40 bg-amber-300/10 py-2.5 text-[10px] font-black uppercase text-amber-100 disabled:opacity-50">{selected ? "Aplicado no provador" : "Aplicar no provador"}</button> : <button disabled={busy || !owned} onClick={() => startTransition(async () => { const result = await equipCosmetic(item.slot, selected ? null : item.id); if (result.success) router.refresh(); setMessage(result.success ? (selected ? `${item.name} desequipado.` : `${item.name} equipado.`) : result.error || "Não foi possível equipar."); })} className={`mt-3 w-full rounded-xl py-2.5 text-[10px] font-black uppercase disabled:opacity-50 ${selected ? "border border-border text-muted" : "bg-accent text-background"}`}>{selected ? "Desequipar" : "Equipar"}</button>}</div></article>;
...
function CosmeticOption({ item, disabled, onChoose }: { item: CosmeticItem; disabled: boolean; onChoose: () => void }) {
  ...
  const isAura = item.slot === "aura";
  const isTitle = item.slot === "title";
  return <button disabled={disabled} onClick={onChoose} className={`group overflow-hidden rounded-2xl border bg-surface text-left transition-transform active:scale-[.98] disabled:opacity-50 ${rarityClass(item.rarity)}`}><div className={`relative flex h-28 items-center justify-center bg-gradient-to-br ${cosmeticVisual(item.assetKey)}`} style={image ? { backgroundImage: `${item.slot === "banner" ? "linear-gradient(rgba(3,14,8,.01),rgba(3,14,8,.18))" : item.slot === "background" ? "linear-gradient(rgba(3,14,8,.02),rgba(3,14,8,.22))" : "linear-gradient(rgba(3,14,8,.1),rgba(3,14,8,.5))"},url(${image})`, backgroundSize: "cover", backgroundPosition: cosmeticBackgroundPosition(item.slot, item.assetKey) } : undefined}>{isTitle && <CosmeticTitlePreview item={item} className="absolute inset-0" />}{(isFrame || isAura) && <PlayerAvatar name="Jogador BQ" clickable={false} frameKey={isFrame ? item.assetKey : null} auraKey={isAura ? item.assetKey : null} className="h-16 w-16 rounded-full bg-[#102819] text-[10px] font-black text-white" />}<div className="cosmetic-card-shimmer absolute inset-0" /></div><span className="block p-3"><span className="text-[8px] font-black uppercase opacity-70">{COSMETIC_SLOT_LABELS[item.slot]}</span><strong className="mt-1 block text-xs text-foreground">{item.name}</strong><span className="mt-2 block text-[9px] font-black uppercase text-accent">Escolher</span></span></button>;
}
