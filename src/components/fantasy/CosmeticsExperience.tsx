...
function CosmeticOption({ item, disabled, onChoose }: { item: CosmeticItem; disabled: boolean; onChoose: () => void }) {
  const image = cosmeticImage(item.assetKey);
  return <button disabled={disabled} onClick={onChoose} className={`group overflow-hidden rounded-2xl border bg-surface text-left transition-transform active:scale-[.98] disabled:opacity-50 ${rarityClass(item.rarity)}`}><div className={`relative h-20 bg-gradient-to-br ${cosmeticVisual(item.assetKey)}`} style={image ? { backgroundImage: `linear-gradient(rgba(3,14,8,.1),rgba(3,14,8,.5)),url(${image})`, backgroundSize: "cover", backgroundPosition: "center" } : undefined}><div className="cosmetic-card-shimmer absolute inset-0" /></div><span className="block p-3"><span className="text-[8px] font-black uppercase opacity-70">{COSMETIC_SLOT_LABELS[item.slot]}</span><strong className="mt-1 block text-xs text-foreground">{item.name}</strong><span className="mt-2 block text-[9px] font-black uppercase text-accent">Escolher</span></span></button>;
}
