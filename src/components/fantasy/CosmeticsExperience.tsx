...
function CosmeticOption({ item, disabled, onChoose }: { item: CosmeticItem; disabled: boolean; onChoose: () => void }) {
  ...
  const isAura = item.slot === "aura";
  return <button disabled={disabled} onClick={onChoose} className={`group overflow-hidden rounded-2xl border bg-surface text-left transition-transform active:scale-[.98] disabled:opacity-50 ${rarityClass(item.rarity)}`}><div className={`relative flex h-28 items-center justify-center bg-gradient-to-br ${cosmeticVisual(item.assetKey)}`} style={image ? { backgroundImage: `${item.slot === "banner" ? "linear-gradient(rgba(3,14,8,.01),rgba(3,14,8,.18))" : "linear-gradient(rgba(3,14,8,.1),rgba(3,14,8,.5))"},url(${image})`, backgroundSize: "cover", backgroundPosition: cosmeticBackgroundPosition(item.slot, item.assetKey) } : undefined}>{(isFrame || isAura) && <PlayerAvatar name="Jogador BQ" clickable={false} frameKey={isFrame ? item.assetKey : null} auraKey={isAura ? item.assetKey : null} className="h-16 w-16 rounded-full bg-[#102819] text-[10px] font-black text-white" />}<div className="cosmetic-card-shimmer absolute inset-0" /></div><span className="block p-3"><span className="text-[8px] font-black uppercase opacity-70">{COSMETIC_SLOT_LABELS[item.slot]}</span><strong className="mt-1 block text-xs text-foreground">{item.name}</strong><span className="mt-2 block text-[9px] font-black uppercase text-accent">Escolher</span></span></button>;
}
