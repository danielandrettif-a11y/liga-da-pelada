...
export function PassRewardPicker({ reward, progress, onClose }: { reward: CosmeticPassReward | null; progress: number; onClose: () => void }) {
  ...
  useEffect(() => {
    // The overlay owns the interaction while open; prevent the page behind it
    // (especially the mobile tab bar) from moving when the user swipes.
  ...
  return <div className="fixed inset-0 z-[100] flex items-center justify-center overflow-hidden bg-black/75 p-3 backdrop-blur-sm sm:p-4" role="dialog" aria-modal="true" aria-label="Escolha da recompensa do Passe" onClick={onClose}>
    <section className="max-h-[min(48rem,calc(100dvh-1.5rem))] w-full max-w-lg overflow-y-auto rounded-3xl border border-accent/35 bg-[#07150d] shadow-[0_24px_70px_rgba(0,0,0,.9)]" onClick={(event) => event.stopPropagation()}>
      ...
      {lockedSecret ? <div className="p-7 text-center"><span className="text-4xl">🔒</span><p className="mt-3 font-black text-white">A casa 40 guarda o prêmio final.</p><p className="mt-2 text-xs leading-5 text-muted">Conclua a trilha para revelar as opções lendárias.</p></div> : reward.rewardType === "card_pack" ? <div className="p-7 text-center"><span className="text-4xl">🎴</span><p className="mt-3 font-black text-white">{unlocked ? "Seu pacote já está no Cartola." : "Avance até esta casa para receber o pacote."}</p><p className="mt-2 text-xs leading-5 text-muted">Pacotes são entregues automaticamente e podem ser abertos no Cartola.</p>{unlocked && <button onClick={() => router.push("/cartola")} className="mt-5 rounded-xl bg-accent px-4 py-3 text-xs font-black uppercase text-background">Abrir Cartola</button>}</div> : <div className="p-4">{chosenId && <div className="mb-3 flex items-center gap-3 rounded-2xl border border-accent/50 bg-accent/15 p-3 text-accent"><CheckCircle2 className="h-6 w-6 shrink-0" /><div><p className="text-[10px] font-black uppercase tracking-wider">Escolha já realizada</p><p className="mt-0.5 text-xs font-bold text-white">Você escolheu: {reward.options.find((item) => item.id === chosenId)?.name || "opção do pacote"}</p><p className="mt-0.5 text-[10px] text-accent/80">Essa decisão é permanente.</p></div></div>}<p className="text-xs leading-5 text-muted">{unlocked ? chosenId ? "Sua escolha é permanente. Os itens recebidos podem ser equipados ou desequipados quando quiser." : hasChoice ? "Toque em uma opção para receber o pacote inteiro. A comparação abaixo mostra exatamente o que entra na sua coleção." : "Esta é uma recompensa exclusiva. Resgate-a para colocar a aura na sua coleção." : "Veja o que cada pacote entrega. Para liberar esta escolha, avance jogando partidas, escalando no Cartola e somando gols e assistências."}</p>
        ...
        <div className="mt-4 grid grid-cols-2 gap-2 sm:gap-3">{reward.options.map((item, index) => <PackageOption key={item.id} item={item} bonus={reward.bonusCosmetic} number={index + 1} selected={chosenId === item.id} hasChosen={Boolean(chosenId)} locked={!unlocked} disabled={pending || Boolean(chosenId)} onChoose={() => choose(item)} />)}</div>
