...
export function PassRewardPicker({ reward, progress, onClose }: { reward: CosmeticPassReward | null; progress: number; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Escolha da recompensa do Passe" onClick={onClose}>
    <section className="max-h-[calc(100dvh-2rem)] w-full max-w-lg overflow-y-auto rounded-3xl border border-accent/35 bg-[#07150d] shadow-[0_24px_70px_rgba(0,0,0,.9)]" onClick={(event) => event.stopPropagation()}>
      ...
      {lockedSecret ? <div className="p-7 text-center"><span className="text-4xl">🔒</span><p className="mt-3 font-black text-white">A casa 40 guarda o prêmio final.</p><p className="mt-2 text-xs leading-5 text-muted">Conclua a trilha para revelar as opções lendárias.</p></div> : reward.rewardType === "card_pack" ? <div className="p-7 text-center"><span className="text-4xl">🎴</span><p className="mt-3 font-black text-white">{unlocked ? "Seu pacote já está no Cartola." : "Avance até esta casa para receber o pacote."}</p><p className="mt-2 text-xs leading-5 text-muted">Pacotes são entregues automaticamente e podem ser abertos no Cartola.</p>{unlocked && <button onClick={() => router.push("/cartola")} className="mt-5 rounded-xl bg-accent px-4 py-3 text-xs font-black uppercase text-background">Abrir Cartola</button>}</div> : <div className="p-4">{chosenId && <div className="mb-3 flex items-center gap-3 rounded-2xl border border-accent/50 bg-accent/15 p-3 text-accent"><CheckCircle2 className="h-6 w-6 shrink-0" /><div><p className="text-[10px] font-black uppercase tracking-wider">Escolha já realizada</p><p className="mt-0.5 text-xs font-bold text-white">Você escolheu: {reward.options.find((item) => item.id === chosenId)?.name || "opção do pacote"}</p><p className="mt-0.5 text-[10px] text-accent/80">Essa decisão é permanente.</p></div></div>}<p className="text-xs leading-5 text-muted">{unlocked ? chosenId ? "Sua escolha é permanente. Os itens recebidos podem ser equipados ou desequipados quando quiser." : hasChoice ? "Toque em uma opção para receber o pacote inteiro. A comparação abaixo mostra exatamente o que entra na sua coleção." : "Esta é uma recompensa exclusiva. Resgate-a para colocar a aura na sua coleção." : "Veja o que cada pacote entrega. Para liberar esta escolha, avance jogando partidas, escalando no Cartola e somando gols e assistências."}</p>
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">{reward.options.map((item, index) => <PackageOption key={item.id} item={item} bonus={reward.bonusCosmetic} number={index + 1} selected={chosenId === item.id} hasChosen={Boolean(chosenId)} locked={!unlocked} disabled={pending || Boolean(chosenId)} onChoose={() => choose(item)} />)}</div>
...
function PackageOption({ item, bonus, number, selected, hasChosen, locked, disabled, onChoose }: { item: CosmeticItem; bonus: CosmeticItem | null; number: number; selected: boolean; hasChosen: boolean; locked: boolean; disabled: boolean; onChoose: () => void }) {
  const isFrame = item.slot === "frame";
  const isNameplate = item.slot === "nameplate";
  return <article className={`overflow-hidden rounded-3xl border bg-[#0b1b11] ${rarityClass(item.rarity)} ${selected ? "ring-2 ring-accent shadow-[0_0_22px_rgba(204,255,0,.28)]" : hasChosen ? "opacity-60" : ""}`}><div className={`relative min-h-32 overflow-hidden bg-gradient-to-br ${cosmeticVisual(item.assetKey)}`}><div className="absolute inset-0 opacity-70" style={(cosmeticImage(item.assetKey) || cosmeticFrameImage(item.assetKey)) ? { backgroundImage: `linear-gradient(rgba(3,14,8,.26),rgba(3,14,8,.72)),url(${cosmeticImage(item.assetKey) || cosmeticFrameImage(item.assetKey)})`, backgroundSize: "cover", backgroundPosition: "center" } : undefined} /><div className="relative flex h-32 items-center justify-center">{isNameplate ? <NameplatePreview item={item} /> : <FramePreview item={isFrame ? item : bonus} />}<span className="absolute left-3 top-3 rounded-full border border-white/15 bg-black/55 px-2 py-1 text-[8px] font-black uppercase text-white">Pacote {number}</span>{selected && <span className="absolute right-3 top-3 rounded-full bg-accent px-2 py-1 text-[8px] font-black uppercase text-background">Sua escolha</span>}</div></div><div className="p-3"><p className="text-[8px] font-black uppercase tracking-wider text-muted">{COSMETIC_SLOT_LABELS[item.slot]}{bonus ? " + moldura" : ""}</p><h3 className="mt-1 text-sm font-black text-white">{item.name}</h3><p className="mt-1 min-h-8 text-[10px] leading-4 text-muted">{item.description}</p>{bonus && <p className="mt-2 rounded-lg bg-amber-300/10 px-2 py-1.5 text-[9px] font-bold text-amber-100">+ {bonus.name}</p>}{selected ? <span className="mt-3 flex items-center justify-center gap-1 rounded-xl bg-accent px-2 py-2.5 text-[9px] font-black uppercase text-background"><CheckCircle2 className="h-3.5 w-3.5" /> Escolhido por você</span> : hasChosen ? <span className="mt-3 flex items-center justify-center rounded-xl border border-white/10 bg-white/5 px-2 py-2.5 text-[9px] font-black uppercase text-muted">Não escolhido</span> : <button type="button" disabled={locked || disabled} onClick={onChoose} className="mt-3 w-full rounded-xl border border-accent/35 bg-accent/10 px-2 py-2.5 text-[9px] font-black uppercase text-accent transition-colors hover:bg-accent hover:text-background disabled:opacity-40">{locked ? "Bloqueado" : "Escolher pacote"}</button>}</div></article>;
}
