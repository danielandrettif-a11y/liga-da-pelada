...
function ShopItem({ item, house40Reached, balance, pending, onBuy }: { item: SeasonPassShopItem; house40Reached: boolean; balance: number; pending: boolean; onBuy: () => void }) {
  ...
  return <article className={`overflow-hidden rounded-2xl border bg-[#0b1b11] ${rarityClass(item.cosmetic.rarity)}`}>
    <div className={`relative h-20 overflow-hidden bg-gradient-to-br ${cosmeticVisual(item.cosmetic.assetKey)}`}>
      {image ? <div className={`absolute inset-0 ${item.cosmetic.slot === "banner" || item.cosmetic.slot === "background" ? "opacity-100" : "opacity-80"}`} style={{ backgroundImage: `${item.cosmetic.slot === "banner" ? "linear-gradient(rgba(3,14,8,.01),rgba(3,14,8,.2))" : item.cosmetic.slot === "background" ? "linear-gradient(rgba(3,14,8,.02),rgba(3,14,8,.26))" : "linear-gradient(rgba(3,14,8,.22),rgba(3,14,8,.74))"},url(${image})`, backgroundSize: "cover", backgroundPosition: "center" }} /> : null}
