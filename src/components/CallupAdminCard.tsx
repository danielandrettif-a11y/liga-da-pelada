...
export function CallupAdminCard({ callup }: { callup: CallupWithEntries | null }) {
      ...
      <div className="glass-card p-4">
        ...
        {!callup ? (
          <form action={create} className="space-y-3">
            <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-accent/10"><CalendarPlus className="h-5 w-5 text-accent" /></div><div><p className="text-sm font-black text-foreground">Abrir convocação</p><p className="text-xs text-muted">15 vagas e 3 na fila</p></div></div>
          ...
          <div className="space-y-3">
            ...
            <button type="button" onClick={buildRound} disabled={loading || confirmed !== 15} className="flex w-full items-center justify-center gap-2 rounded-xl bg-accent py-3 text-sm font-black text-background disabled:opacity-40"><Lock className="h-4 w-4" /> Montar rodada com os 15</button>
          </div>
        )}
      </div>
    </section>
