...
export function OpenCallupBanner({
  ...
  let ctaClass = "bg-accent text-background";
  ...
  if (isConfirmed) {
    ...
    ctaLabel = "Ver vaga";
    ctaIcon = <CheckCircle2 className="h-3.5 w-3.5" />;
    ctaClass = "bg-success text-background shadow-[0_0_20px_rgba(34,197,94,.25)]";
  } else if (isWaiting) {
    ...
    ctaLabel = "Acompanhar";
    ctaIcon = <Clock className="h-3.5 w-3.5" />;
    ctaClass = "bg-warning text-background shadow-[0_0_20px_rgba(234,179,8,.25)]";
  } else if (isFull) {
    ...
    ctaIcon = <Users className="h-3.5 w-3.5" />;
    ctaClass = "bg-warning text-background shadow-[0_0_20px_rgba(234,179,8,.25)]";
  }
  ...
  return (
    <div className="group relative isolate flex h-full min-h-[350px] sm:min-h-[360px] flex-col justify-between overflow-hidden rounded-[28px] border border-accent/45 bg-[#07150d] p-5 shadow-[0_18px_45px_rgba(0,0,0,.34),0_0_28px_rgba(204,255,0,.08)] sm:p-6">
      ...
      <div className="relative z-10 flex h-full flex-col justify-between gap-3">
        ...
        {/* Mensagem e Ícone */}
        <Link href="/convocacao" className="flex items-start gap-4">
          <div
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-white/10 bg-white/[0.04] p-3">
            ...
            {callup.stadiumMapUrl && (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex items-center gap-1 rounded-lg border border-accent/30 bg-accent/10 px-2.5 py-1.5 text-[10px] font-black uppercase text-accent hover:bg-accent/20 transition-colors shrink-0"
              >
        ...
        <div className="grid grid-cols-[1fr_auto] items-center gap-3 border-t border-white/10 pt-3">
          ...
          <Link
            href="/convocacao"
            className={`flex items-center gap-1 rounded-xl px-3.5 py-2.5 text-[10px] font-black uppercase transition-transform active:scale-95 ${ctaClass}`}
          >
            {ctaLabel} {ctaIcon}
          </Link>
        </div>
