      ...
      <section className="glass-card overflow-hidden">
        {initialData.snapshots.length === 0 ? (
          ...
          <div key={item.playerId} className="border-b border-border p-4 last:border-0">
            ...
            {item.comparison && <p className="mt-1 text-[10px] font-bold text-accent">vs v5: OVR {item.comparison.overallDelta >= 0 ? "+" : ""}{item.comparison.overallDelta.toFixed(1)} · DEF {item.comparison.defDelta >= 0 ? "+" : ""}{item.comparison.defDelta.toFixed(1)} · ALA/MEI {item.comparison.alaMeiDelta >= 0 ? "+" : ""}{item.comparison.alaMeiDelta.toFixed(1)} · ATA {item.comparison.ataDelta >= 0 ? "+" : ""}{item.comparison.ataDelta.toFixed(1)}</p>}
