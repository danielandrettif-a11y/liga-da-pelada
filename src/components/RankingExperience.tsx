...
export function RankingExperience({ data, currentPlayerId, initialView = "season", initialFilter = "general" }: Props) {
  const safeInitialFilter = RANKING_FILTERS.includes(initialFilter as RankingFilter) ? initialFilter as RankingFilter : "general";
  const [view, setView] = useUrlState({ key: "period", initialValue: initialView, defaultValue: "season", allowedValues: RANKING_VIEWS });
  const [filter, setFilter] = useUrlState({ key: "filter", initialValue: safeInitialFilter, defaultValue: "general", allowedValues: RANKING_FILTERS, history: "replace" });
  ...
  async function handleShare() {
    ...
    setShareMessage("");
    try {
      // O gerador usa canvas e imagens em alta resolução; só entra no bundle
      // quando a pessoa realmente pede a arte para compartilhar.
      const { createPodiumStory: createStory } = await import("@/lib/ranking-story");
      ...
      const blob = await createStory(podium, data.seasonLabel, roundLabel, periodTitle);
  ...
  return (
    <div className={`space-y-5 ${pinnedEntry ? "pb-24" : "pb-16"}`}>
      ...
      <div className="grid grid-cols-3 rounded-xl border border-border bg-surface p-1" role="tablist" aria-label="Período do ranking">
        <button
          type="button"
          role="tab"
