...
export function DeleteRoundButton({ round, redirectTo }: { round: { id: string; number: number; round_type: string; date: string; playersCount: number; matchesCount: number }; redirectTo?: string }) {
  ...
  return <>
    ...
    {open && <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/75 p-3 sm:items-center" onClick={(event) => event.stopPropagation()}>
      <div className="w-full max-w-md rounded-3xl border border-danger/30 bg-background p-5 shadow-2xl">
