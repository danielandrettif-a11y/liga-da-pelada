  ...
  return <>
    ...
    {open && <div className="mobile-dialog-backdrop bg-black/75 backdrop-blur-sm" onMouseDown={(event) => event.target === event.currentTarget && !loading && setOpen(false)}>
      <div role="dialog" aria-modal="true" aria-label="Excluir rodada" className="mobile-dialog-panel max-w-md rounded-3xl border border-danger/30 bg-background p-5 shadow-2xl animate-fade-in-up">
        ...
        {round.round_type !== "friendly" && <p className="mt-2 rounded-xl border border-warning/25 bg-warning/10 p-3 text-xs font-semibold leading-relaxed text-warning">Se esta rodada passou pelo Cartola, precos, pontos e patrimonio serao restaurados automaticamente para o estado anterior. Por seguranca, apague sempre da rodada mais recente para a mais antiga.</p>}
        <label className="mt-4 block text-[10px] font-black uppercase tracking-wider text-muted">Digite EXCLUIR<input value={confirmation} onChange={(event) => setConfirmation(event.target.value)} placeholder="EXCLUIR" className="mt-2 w-full rounded-xl border border-border bg-surface px-4 py-3 text-sm font-bold text-foreground outline-none focus:border-danger placeholder:text-muted/40" /></label>
        {error && <p className="mt-3 text-xs font-semibold text-danger">{error}</p>}
        <div className="mt-5 grid grid-cols-2 gap-3"><button type="button" onClick={() => setOpen(false)} className="rounded-xl border border-border py-3 text-sm font-bold text-foreground">Cancelar</button><button type="button" disabled={loading || confirmation.trim().toUpperCase() !== "EXCLUIR"} onClick={handleDelete} className="rounded-xl bg-danger py-3 text-sm font-black text-white disabled:opacity-40">{loading ? "Excluindo..." : "Apagar tudo"}</button></div>
      </div>
    </div>}
  </>;
}
