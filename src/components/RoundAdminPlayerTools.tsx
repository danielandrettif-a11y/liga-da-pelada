...
export function RoundAdminPlayerTools({ roundId, status, participants, teams, allPlayers }: { roundId: string; status: string; participants: Participant[]; teams: Team[]; allPlayers: Player[] }) {
  ...
  function zeroPoints() {
    if (status !== "finished" || !zeroPlayerId) return;
    if (window.prompt("Digite ZERAR para confirmar a correção desta rodada.") !== "ZERAR") return;
    setMessage("");
    startTransition(async () => {
      const result = await zeroPlayerRoundPoints(roundId, zeroPlayerId, zeroReason);
      setMessage(result.success ? "Pontuação do jogador zerada nesta rodada." : result.error || "Não foi possível zerar a pontuação.");
      if (result.success) { setZeroPlayerId(""); router.refresh(); }
    });
  }
        ...
        <div className="glass-card space-y-3 p-4 animate-fade-in">
          ...
          {status === "finished" && (
            <div className="space-y-2 border-t border-border pt-3">
              <div>
                <p className="text-[10px] font-black uppercase text-muted">Zerar pontuação de um jogador</p>
                <p className="mt-1 text-[10px] text-muted">Corrige somente esta rodada; o placar e os demais jogadores permanecem iguais.</p>
              </div>
              <label className="block text-[10px] font-black uppercase text-muted">Jogador que não participou<select value={zeroPlayerId} onChange={(event) => setZeroPlayerId(event.target.value)} className="mt-1 h-12 w-full rounded-xl border border-border bg-background px-3 text-sm text-foreground"><option value="">Escolha</option>{participants.filter((entry) => entry.players).map((entry) => <option key={entry.player_id} value={entry.player_id}>{entry.players?.name}</option>)}</select></label>
              <label className="block text-[10px] font-black uppercase text-muted">Motivo (opcional)<input value={zeroReason} onChange={(event) => setZeroReason(event.target.value)} maxLength={200} className="mt-1 h-12 w-full rounded-xl border border-border bg-background px-3 text-sm text-foreground" /></label>
              <button type="button" disabled={pending || !zeroPlayerId} onClick={zeroPoints} className="flex w-full items-center justify-center gap-2 rounded-xl border border-danger/40 bg-danger/10 py-3 text-xs font-black text-danger disabled:opacity-40">{pending && <Loader2 className="h-4 w-4 animate-spin" />}Zerar pontos desta rodada</button>
            </div>
          )}
