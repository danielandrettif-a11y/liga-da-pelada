...
export default async function FantasyUserRoundHistoryPage({ params }: { params: Promise<{ userId: string; roundId: string }> }) {
  ...
  return <div className="space-y-5">
    <header><Link href={`/cartola/ranking/${userId}`} className="text-xs font-bold text-accent">← Histórico de {name}</Link><h1 className="mt-3 text-xl font-black text-foreground">Rodada {String(data.round?.number || 0).padStart(2, "0")}</h1><p className="mt-1 text-xs text-muted">{data.round?.date} · total {Number(data.lineup.total_points || 0).toFixed(1)} pts</p></header>
