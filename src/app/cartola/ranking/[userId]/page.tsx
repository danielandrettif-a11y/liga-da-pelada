...
export default async function FantasyUserHistoryPage({ params }: { params: Promise<{ userId: string }> }) {
  ...
  return <div className="space-y-5">
    <header>
      <Link href="/cartola/ranking" className="text-xs font-bold text-accent">← Voltar ao ranking</Link>
      <div className="mt-4 flex items-center gap-3">
        <PlayerAvatar name={name} avatarUrl={avatarUrl} className="h-14 w-14 rounded-full bg-surface text-base font-black text-accent" />
        <div><h1 className="text-xl font-black text-foreground">{name}</h1><p className="text-xs text-muted">Histórico da temporada no Cartola</p></div>
      </div>
    </header>
    <section className="grid grid-cols-3 divide-x divide-border rounded-2xl border border-border bg-surface p-3 text-center">
      <Stat label="Pontos" value={data.account.totalPoints.toFixed(1)} />
      <Stat label="Rodadas" value={String(data.account.roundsPlayed)} />
      <Stat label="Patrimônio" value={`C$ ${data.account.currentBudget.toFixed(2)}`} />
    </section>
    <section className="space-y-2">
      <h2 className="px-1 text-xs font-black uppercase tracking-wider text-muted">Rodadas contabilizadas</h2>
      ...
      </Link>) : <p className="glass-card p-6 text-center text-sm text-muted">Nenhuma rodada processada ainda.</p>}
    </section>
  </div>;
}

function Stat({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0 px-2"><p className="text-[8px] font-black uppercase tracking-wider text-muted">{label}</p><p className="mt-1 truncate text-xs font-black text-foreground">{value}</p></div>;
}
