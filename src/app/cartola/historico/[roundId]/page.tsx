...
export default async function FantasyRoundHistoryPage({ params }: { params: Promise<{ roundId: string }> }) {
  ...
  const breakdown = (lineup.score_breakdown || {}) as Record<string, unknown>;
  ...
  return <div className="space-y-5">
    <header><Link href="/cartola/historico" className="text-xs font-bold text-accent">← Histórico</Link><h1 className="mt-3 text-xl font-black text-foreground">Rodada {(fantasyRound.round as any)?.number}</h1><p className="text-xs text-muted">Escalação e pontuação congeladas.</p></header>
    <div className="grid grid-cols-2 gap-2"><Metric label="Posição" value={lineup.round_position ? `${lineup.round_position}º` : "—"}/><Metric label="Total" value={Number(lineup.total_points || 0).toFixed(1)}/></div>
