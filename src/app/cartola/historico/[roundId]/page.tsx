...
export default async function FantasyRoundHistoryPage({ params }: { params: Promise<{ roundId: string }> }) {
  ...
  return <div className="space-y-5">
    ...
    <section className="space-y-2"><h2 className="text-xs font-black uppercase text-muted">Escalação</h2>{(lineup.fantasy_lineup_players || []).map((item: any) => { const name = item.player_name_locked || item.players?.name || "Jogador"; const captain = item.player_id === lineup.captain_player_id; return <div key={item.id} className="glass-card flex items-center gap-3 p-3"><PlayerAvatar name={name} avatarUrl={item.avatar_url_locked || item.players?.avatar_url} className="h-10 w-10 rounded-full bg-surface text-xs font-black text-accent"/><div className="min-w-0 flex-1"><p className="truncate text-sm font-black text-foreground">{name}{captain ? " · Capitão" : ""}</p><p className="text-[10px] text-muted">Base {Number(item.base_points || 0).toFixed(1)} · bônus {Number(item.captain_bonus || 0).toFixed(1)}</p></div><strong className="text-base text-accent">{Number(item.total_points || 0).toFixed(1)}</strong></div>; })}</section>
    <section className="glass-card space-y-3 p-4"><h2 className="text-xs font-black uppercase text-muted">Composição da pontuação</h2><ScoreRow label="Pontos-base dos jogadores" value={Number(breakdown.playersBase || 0)}/><ScoreRow label="Bônus do capitão" value={Number(breakdown.captainBonus || 0)}/><ScoreRow label={`Artilheiro${predictions.topScorer?.playerName ? ` · ${predictions.topScorer.playerName}` : ""}`} value={Number(breakdown.topScorer || 0)}/><ScoreRow label={`Garçom${predictions.topAssist?.playerName ? ` · ${predictions.topAssist.playerName}` : ""}`} value={Number(breakdown.topAssist || 0)}/><ScoreRow label={`${challengeType ? CHALLENGE_LABELS[challengeType] : "Desafio"}${challenge.playerName ? ` · ${challenge.playerName}` : ""}`} value={Number(breakdown.challenge || 0)}/></section>
  </div>;
}
