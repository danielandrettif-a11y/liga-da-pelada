...
export default async function JogadorPerfilPage({ params, searchParams }: PageProps<"/jogadores/[id]">) {
  ...
  return (
    <div className="relative -mx-4 -mt-4 min-h-[calc(100dvh-3.5rem)] px-4 pb-4 pt-4">
      ...
      {isPlayable && <section className="rounded-2xl border border-amber-300/25 bg-gradient-to-br from-amber-300/10 via-surface/85 to-surface/70 p-4 shadow-[0_0_24px_rgba(251,191,36,.06)]"><div className="mb-3 flex items-center gap-2"><Trophy className="h-4 w-4 text-amber-300" /><div><h3 className="text-xs font-black uppercase tracking-wider text-foreground">Prêmios individuais</h3><p className="mt-0.5 text-[10px] text-muted">Destaques mensais, da rodada e títulos da temporada Ranked.</p></div></div><MonthlyAwards awards={monthlyAwards} />{(awardSeasons.length > 0 || monthlyAwards.length === 0) && <PlayerAwards seasons={awardSeasons} />}</section>}
