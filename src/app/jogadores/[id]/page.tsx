import { getPlayer, getPlayerAwardSeasons, getPlayerGoalsByClub, getPlayerMonthlyAwards, getPlayerPlaytime, getPlayerRoundHistory } from "@/lib/actions/players";
...
function rankedPointDescription(row: HistoryRow) {
  ...
  return parts.length > 0
    ? `${parts.join(" · ")} = ${formatPointValue(total)} pts`
      ...
      {rows.map((row, index) => (
        <Link key={row.id} href={`/rodadas/${row.round_id}`} className={`flex items-center gap-4 px-4 py-4 hover:bg-surface-hover ${index < rows.length - 1 ? "border-b border-border" : ""}`}>
          ...
          <div className="min-w-0 flex-1">
            <p className="mb-1 text-xs text-muted">{row.rounds ? formatDateShort(row.rounds.date) : ""}</p>
            <div className="flex items-center gap-3">
              <span className="flex items-center gap-1 text-sm font-bold text-foreground"><Football className="h-3 w-3 text-muted" />{row.goals}</span>
...
export default async function JogadorPerfilPage({ params, searchParams }: PageProps<"/jogadores/[id]">) {
  ...
  return (
    <div className="relative -mx-4 -mt-4 min-h-[calc(100dvh-3.5rem)] px-4 pb-4 pt-4">
      ...
      {(isPlayable || monthlyAwards.length > 0) && <section className="rounded-2xl border border-amber-300/25 bg-gradient-to-br from-amber-300/10 via-surface/85 to-surface/70 p-4 shadow-[0_0_24px_rgba(251,191,36,.06)]"><div className="mb-3 flex items-center gap-2"><Trophy className="h-4 w-4 text-amber-300" /><div><h3 className="text-xs font-black uppercase tracking-wider text-foreground">Prêmios individuais</h3><p className="mt-0.5 text-[10px] text-muted">Destaques mensais, da rodada e títulos da temporada Ranked.</p></div></div><MonthlyAwards awards={monthlyAwards} />{isPlayable && (awardSeasons.length > 0 || monthlyAwards.length === 0) && <PlayerAwards seasons={awardSeasons} />}</section>}
