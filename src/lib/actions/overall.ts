...
const FORMULA_KEY = "adaptive-v3-defensive-timing-shadow";
...
async function loadOverallHistory(client: any) {
  ...
  const [{ data: members, error: membersError }, { data: rounds, error: roundsError }] = await Promise.all([
    client.from("league_members")
      .select("players!inner(id, player_profile, overall_seed_mode, is_goalkeeper, is_selectable, member_category)")
  ...
  const players = (members || [])
    ...
    .map((player: any) => ({ id: player.id, playerProfile: player.player_profile, overallSeedMode: player.overall_seed_mode, isGoalkeeper: Boolean(player.is_goalkeeper) })) satisfies OverallPlayer[];
  ...
  return buildOverallHistoryInput({ players, rounds: rounds || [], zeroPointOverrides: overrides || [] });
}

type PositionMap = Record<OverallRole, number>;
type RecentRound = { date: string; goals: number; assists: number; goalsConceded: number; attackingScore: number; defensiveScore: number; timingQuality: "exact" | "fallback"; positions: PositionMap };
...
export async function getOverallShadowAdminData(): Promise<OverallShadowAdminData | null> {
  ...
  if (!formula) return { latestRun: null, snapshots: [] };
  ...
  const [{ data: rows, error: snapshotsError }, { data: breakdownRows, error: breakdownError }] = await Promise.all([
    ...
    database.from("player_overall_round_breakdowns").select("player_id, round_date, round_index, goals, assists, goals_conceded, attacking_score, defensive_score, timing_quality, positions").eq("calculation_run_id", latestRun.id).order("round_index", { ascending: false }),
  ]);
...
export async function recalculateOverallShadow() {
  ...
  try {
    ...
    if (formulaError || !formula) throw new Error("A fórmula v3 não foi encontrada. Confirme a migration 163.");
    ...
    const rows = calculation.snapshots.map((snapshot) => ({
      ...
      data_quality: { mode: "shadow", goal_timing: "first_conceded_goal_with_legacy_fallback", scoring_unit: "weekly_round", seed_mode: source.players.find((player) => player.id === snapshot.playerId)?.overallSeedMode || "observed", scout_totals: snapshot.scoutTotals, position_confidence: Object.fromEntries(Object.entries(snapshot.positions).map(([role, position]) => [role, position.confidence])) },
    }));
    if (rows.length) {
