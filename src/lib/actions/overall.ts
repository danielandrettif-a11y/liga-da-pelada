...
const FORMULA_KEY = "adaptive-v5-characteristics-shadow";
const COMPARISON_FORMULA_KEY = "adaptive-v4-role-responsibility-shadow";
...
export async function recalculateOverallShadow() {
  ...
  try {
    ...
    if (formulaError || !formula) throw new Error("A fórmula v5 não foi encontrada. Confirme a migration 165.");
    const source = await loadOverallHistory(database);
