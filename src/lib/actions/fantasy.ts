...
export async function getFantasyDashboard() {
  ...
  if (liveEvents?.length) {
    ...
    for (const [playerId, current] of currentStats) {
      ...
      current.draws = Number(official?.draws || 0);
  ...
  const liveStats = projectFantasyLiveStats(
    ...
    scoringSettings,
    { suppressGoalkeeperRewards: Boolean(displayRound?.suppress_goalkeeper_rewards) },
  );
...
export async function getFantasyPlayerDetail(playerId: string, fantasyRoundId?: string) {
  ...
  if (scoringRound?.round_id) {
    ...
    const breakdown = [
      ...
      { key: "draws", label: "Empates", count: current.draws || 0, unitPoints: liveSettings.drawPoints, points: (current.draws || 0) * liveSettings.drawPoints, icon: "🤝" },
...
export async function updateFantasySettings(values: Partial<FantasySettings>) {
  ...
  const changesCanonicalScout = [
    values.goalPoints, values.assistPoints, values.winPoints, values.drawPoints,
    values.lossPoints, values.ownGoalPoints, values.goalkeeperAppearancePoints,
    values.goalConcededPoints,
...
export async function getFantasyAdminData() {
  ...
  return {
    settings: settings || {
      ...
      competitive_price_floor: DEFAULT_FANTASY_SETTINGS.competitivePriceFloor,
      competitive_price_ceiling: DEFAULT_FANTASY_SETTINGS.competitivePriceCeiling,
      competitive_price_curve: DEFAULT_FANTASY_SETTINGS.competitivePriceCurve,
      market_round_weight: DEFAULT_FANTASY_SETTINGS.marketRoundWeight,
      market_attendance_weight: DEFAULT_FANTASY_SETTINGS.marketAttendanceWeight,
      market_reprice_strength: DEFAULT_FANTASY_SETTINGS.marketRepriceStrength,
      market_initial_up_cap: DEFAULT_FANTASY_SETTINGS.marketInitialUpCap,
      market_initial_down_cap: DEFAULT_FANTASY_SETTINGS.marketInitialDownCap,
      market_cap_step: DEFAULT_FANTASY_SETTINGS.marketCapStep,
      budget_soft_cap_multiplier: DEFAULT_FANTASY_SETTINGS.budgetSoftCapMultiplier,
      budget_hard_cap_multiplier: DEFAULT_FANTASY_SETTINGS.budgetHardCapMultiplier,
      budget_excess_retention: DEFAULT_FANTASY_SETTINGS.budgetExcessRetention,
  ...
  return {
    ...
    statsByPlayer: projection
      ? Object.fromEntries(playerIds.map((playerId: string) => {
          const stat = live?.statsByPlayer.get(playerId);
